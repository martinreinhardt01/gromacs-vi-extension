    ...
    if (ir->efep != efepNO)
    {
        ...
        if ((fep->sc_alpha > 0) && (!fep->bScCoul))
        {
        	if (fep->var_morph)
        	{
        		//for (i = 0; i < fep->n_lambda; i++)
        		//{
                //    sprintf(warn_buf, "For state %d, vdw-lambdas (%f) is changing with vdw softcore, while coul-lambdas (%f) is nonzero without coulomb softcore: Ok for variational morphing, but otherwise may lead to crashes.", i, fep->all_lambda[efptVDW][i],
            ...
            WriteMdpHeader writeMdpHeader, warninp_t wi)
{
    ...
    fep->dh_hist_size       = get_eint(&inp, "dh_hist_size", 0, wi);
    fep->dh_hist_spacing    = get_ereal(&inp, "dh_hist_spacing", 0.1, wi);
    fep->var_morph          = get_eint(&inp, "variational-morphing", 0, wi);
    fep->deltag_est         = get_ereal(&inp, "deltag-estimate", 0.0, wi);
    fep->smoothing          = get_ereal(&inp, "smoothing-factor", 2.0, wi);
    fep->linear_test        = get_eint(&inp, "linear-test", 0, wi);
