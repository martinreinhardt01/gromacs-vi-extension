                                             ...
                                             t_nrnb                     *nrnb)
{
    ...
    real             dvdl_AB[2]      = { 0 }; // var_morph softcore
    kernel_data.flags     = donb_flags;
    kernel_data.lambda    = lambda;
    kernel_data.dvdl      = dvdl_nb;
    kernel_data.dvdl_AB   = dvdl_AB; // var_morph track dvdl contributions separately
    kernel_data.var_morph = fepvals->var_morph;

    kernel_data.energygrp_elec    = enerd->grpp.ener[egCOULSR].data();
    kernel_data.energygrp_vdw     = enerd->grpp.ener[egLJSR].data();
    kernel_data.energygrpA_total  = enerd->grpp.enerA.data(); // var_morph: energies tracked separately
    kernel_data.energygrpB_total  = enerd->grpp.enerB.data();
...
#pragma omp parallel for schedule(static) num_threads(nbl_fep.ssize())
    ...
    if (fepvals->sc_alpha != 0)
    {
        ...
        {
        	enerd->dvdl_AB[0] += dvdl_AB[0];
        	enerd->dvdl_AB[1] += dvdl_AB[1];
        }
    }
    else
    {
    ...
    if (fepvals->n_lambda > 0 && stepWork.computeDhdl && fepvals->sc_alpha != 0)
    {
        ...
        kernel_data.energygrpA_total  = enerd->foreign_grpp.enerA.data();
        kernel_data.energygrpB_total  = enerd->foreign_grpp.enerB.data();
...
#pragma omp parallel for schedule(static) num_threads(nbl_fep.ssize())
            ...
            /* For soft-core: as the end states are lambda dependent, these
             * have to be tracked separately for each state in order to
             * evaluate configurations with Hamiltonians from foreign states
            */
            if(fepvals->var_morph){
            	enerd->enerpart_lambdaA[i] += enerd->foreign_term[F_EPOTA];
            	enerd->enerpart_lambdaB[i] += enerd->foreign_term[F_EPOTB];
            }
            else{
