...
#define  NSTATES  2
    ...
    real       *dvdl_AB         = kernel_data->dvdl_AB; // var_morph: separate tracking of dhdl
    ...
    real       *Va              = kernel_data->energygrpA_total; // var_morph separate tracking
    real       *Vb              = kernel_data->energygrpB_total; // separate tracking of end state energies
    ...
    int         var_morph       = kernel_data->var_morph;
    ...
    SCReal dvdl_AB_loc[NSTATES]  = {0.}; /* var_morph: derivatives of the end states with */
    dvdl_AB_loc[0]               = 0.;   /* respect to lambda in soft-core cases */
    dvdl_AB_loc[1]               = 0.;   /* local copy of it /*
    ...
    real * gmx_restrict  fA     = &(forceWithShiftForces->forceA()[0][0]);
    real * gmx_restrict  fB     = &(forceWithShiftForces->forceB()[0][0]);
    ...
    for (int n = 0; n < nri; n++)
    {
        ...
        real       fixAB[NSTATES], fiyAB[NSTATES], fizAB[NSTATES];
        fixAB[0]         = 0;
        fixAB[1]         = 0;
        fiyAB[0]         = 0;
        fiyAB[1]         = 0;
        ...
        for (int k = nj0; k < nj1; k++)
        {
            ...
            real FscalAB[NSTATES]; // Assembles A and B states separately
                                    // without multiplication by lambda value right away
            FscalAB[0] = 0.;
            FscalAB[1] = 0.;
            ...
            if (nlist->excl_fep == nullptr || nlist->excl_fep[k])
            {
                ...
                for (int i = 0; i < NSTATES; i++)
                {
                    if( var_morph )
                    {
                    	vtotAB[i]  += Vcoul[i];
                    	vtotAB[i]  += Vvdw[i];
                        FscalAB[i] += FscalC[i]*rpm2;
                        FscalAB[i] += FscalV[i]*rpm2;

                        if (useSoftCore)
                        {
                        	dvdl_AB_loc[i] += alpha_coul_eff*dlfac_coul[i]*FscalC[i]*sigma_pow[i];
                        	dvdl_AB_loc[i] += alpha_vdw_eff*dlfac_vdw[i]*FscalV[i]*sigma_pow[i];
                        }
                    }
            ...
            {
            	if( var_morph ){
            		real txAB[NSTATES], tyAB[NSTATES], tzAB[NSTATES];
                    for (int i = 0; i < NSTATES; i++)
                    {
                    	txAB[i]   = FscalAB[i]*dx;
                    	tyAB[i]   = FscalAB[i]*dy;
...
#pragma omp atomic
            ...
            // for variational morphing we need the energies at every step to calculate the forces
            if ( var_morph ){
                int ggid  = gid[n];
...
#pragma omp atomic
    dvdl_AB[0]         += dvdl_AB_loc[0];
#pragma omp atomic
    dvdl_AB[1]         += dvdl_AB_loc[1];
