                  ...
                  const DDBalanceRegionHandler             &ddBalanceRegionHandler)
{
    ...
    int var_morph = ir->fepvals->var_morph;

    //write_dhdl_full(enerd, *ir->fepvals);
...

    {
        ...
        //write_forces(md, forceOutputs, 0);
    ...
    //write_forces(md, forceOutputs, 5);
...
#pragma omp parallel for num_threads(nthreads) schedule(static)
            ...
            if (EEL_PME_EWALD(fr->ic->eeltype) && fr->n_tpi == 0)
            {
            	...
            	{
            		ewaldOutput.Va +=
                            ewald_charge_correction(cr, fr, 0., box,
                                                    &ewaldOutput.dvdl[efptCOUL],
                                                    ewaldOutput.vir_q);
            		ewaldOutput.Vb +=
                            ewald_charge_correction(cr, fr, 1., box,
                                                    &ewaldOutput.dvdl[efptCOUL],
                                                    ewaldOutput.vir_q);
            	}
            	else
            ...
            if (computePmeOnCpu)
            {
                ...
                if (fr->n_tpi == 0 || stepWork.stateChanged)
                {
                    ...
                    status = gmx_pme_do(fr->pmedata,
                                        ...
                                        forceWithVirial.forceA_,
                                        forceWithVirial.forceB_,
                                        md->chargeA, md->chargeB,
                                        ...
                                        pme_flags, ir->fepvals->var_morph);
                ...
                //write_dhdl_to_output(&ewaldOutput, 3);
                if (fr->n_tpi > 0)
                {
        ...
        if (fr->ic->eeltype == eelEWALD)
        {
        	if( var_morph )
        	{
        		Va = do_ewald(ir, x, as_rvec_array(forceWithVirial.force_.data()),
                        md->chargeA, md->chargeB,
                        box, cr, md->homenr,
                        ewaldOutput.vir_q, fr->ic->ewaldcoeff_q,
                        0., &ewaldOutput.dvdl[efptCOUL],
                        fr->ewald_table);
        		Vb = do_ewald(ir, x, as_rvec_array(forceWithVirial.force_.data()),
                        md->chargeA, md->chargeB,
                        box, cr, md->homenr,
                        ewaldOutput.vir_q, fr->ic->ewaldcoeff_q,
                        1., &ewaldOutput.dvdl[efptCOUL],
                        fr->ewald_table);
        	}
        	else
        	{
        ...
        //write_dhdl_to_output(&ewaldOutput, 5);
        ...
        if( !var_morph ){
			...
			enerd->term[F_EPOTA]      += Va + ewaldOutput.Va;
			enerd->term[F_EPOTB]      += Vb + ewaldOutput.Vb;
			enerd->term[F_VM_COUL_CORR] += Vb + ewaldOutput.Vb - (Va + ewaldOutput.Va);
        }
    ...
    //write_forces(md, forceOutputs, 9);
