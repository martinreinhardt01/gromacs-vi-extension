...
setupForceOutputs(t_forcerec                          *fr,
				  ...
				  gmx::ArrayRefWithPadding<gmx::RVec>  forceA,
				  gmx::ArrayRefWithPadding<gmx::RVec>  forceB,
                  const StepWorkload                  &stepWork,
                  gmx_wallcycle_t                      wcycle)
{
    ...
    gmx::ForceWithShiftForces forceWithShiftForces(force, forceA, forceB, stepWork.computeVirial, fr->shiftForces);
    ...
    if (stepWork.computeForces)
    {
        ...
        clear_rvecs_omp(fr->natoms_force_constr,
                        as_rvec_array(forceWithShiftForces.forceA().data()));
        clear_rvecs_omp(fr->natoms_force_constr,
                        as_rvec_array(forceWithShiftForces.forceB().data()));
    }
    ...
    gmx::ForceWithVirial forceWithVirial(useSeparateForceWithVirialBuffer ?
										 ...
										 forceA.unpaddedArrayRef(), forceB.unpaddedArrayRef(), stepWork.computeVirial);
    ...
    if (useSeparateForceWithVirialBuffer)
    {
        ...
        clear_rvecs_omp(forceWithVirial.forceA_.size(), as_rvec_array(forceWithVirial.forceA_.data()));
        clear_rvecs_omp(forceWithVirial.forceB_.size(), as_rvec_array(forceWithVirial.forceB_.data()));
    }
...
// For developers: check individual force contributions
void write_forces_to_log(FILE *fplog, const t_mdatoms *md, ForceOutputs *forceOut)
{
    //real c = 0.01;
	rvec *f  = as_rvec_array(forceOut->forceWithShiftForces().force().data());
	rvec *fA = as_rvec_array(forceOut->forceWithShiftForces().forceA().data());
	rvec *fB = as_rvec_array(forceOut->forceWithShiftForces().forceB().data());

	fprintf(fplog, "\n \n  step  \n ");

    for (int i=0; i< md->nr; i++){
    	//if( (abs(fA[i][0]) > c * abs(f[i][0]) || abs(fA[i][1]) > c * abs(f[i][1]) || abs(fA[i][2]) > c * abs(f[i][2])) ){// && ( step % 10 == 0 )  ){
    	    fprintf(fplog, " atom %6d  f  %12.5e %12.5e %12.5e\n", i, f[i][0], f[i][1], f[i][2]);
    	    fprintf(fplog, " atom %6d  fA %12.5e %12.5e %12.5e\n", i, fA[i][0], fA[i][1], fA[i][2]);
    	    fprintf(fplog, " atom %6d  fB %12.5e %12.5e %12.5e\n", i, fB[i][0], fB[i][1], fB[i][2]);
    	//}
    }
}
// For developers: check individual contributions to dhdl
void write_dhdl_to_log(FILE *fplog, gmx_enerdata_t *enerd)
{

    fprintf(fplog, " dvdl_lin[efptVdW], dvdl_lin[efptCoul]: %12.5e %12.5e\n", enerd->dvdl_lin[efptVDW], enerd->dvdl_lin[efptCOUL]);
    fprintf(fplog, " enerd->term[F_EPOTB]-enerd->term[F_EPOTA] : %12.5e \n", enerd->term[F_EPOTB] - enerd->term[F_EPOTA]);
	fprintf(fplog, "\n \n  next step \n \n");
}

/* The total forces that differ between A and B have been collected in fA and fB. These are combined
 * to the VI force and added to all the remaining forces (i.e., the ones that are identical in A and B).
 * For the linear test, this is requires
 */
void reduce_AB_f_to_f(const t_mdatoms *md, ForceOutputs *forceOut, gmx_enerdata_t *enerd, real *lambda, t_lambda *fepvals){
	...
	// If it is one of the end states, don't put numbers through ln ( exp ( xx ) ) to avoid numerical inaccuracies
	// In this case do not need to evaluate the energies in A and B
	// state A
	if(lam < 0.00001)
	{
		for (int i=0; i< md->nr; i++){
	...
	else if(fepvals->var_morph && ! fepvals->linear_test)
    {
    	...
    	Va = Va_get[0] + enerd->term[F_EPOTA];
    	Vb = Vb_get[0] + enerd->term[F_EPOTB];

        if (fepvals->sc_alpha != 0){
        	// long range dispersion correction term for energy
        	// in case the end states are lambda dependent
            Vb += enerd->term[F_VM_LIN_CORR];
        }

		Vb -= deltag_est;

    	// To avoid overly large numbers in exponent: Setting the larger potential to zero and the smaller one to the difference < 0
		if (Va > Vb)
		{
			Vb -= Va;
			Va = 0.;
		...
		// calculate the exponential weighting prefactors of the forces
		double gA = (1.-lam) * exp( -s * Va  / kbT );
		double gB = lam     * exp( -s * Vb  / kbT );

		double bracket = gA + gB;

		for (int i=0; i< md->nr; i++){
			for (int dim=0; dim<3; dim++){
				// add to non-lambda dependent force and combine to total force
				f[i][dim]  += 1. / bracket * ( gA * fA[i][dim] + gB * fB[i][dim] );
				fA[i][dim] = 0.;
				fB[i][dim] = 0.;
			}
		}
	}
	/* if linear test: fA and fB have been tracked separately, but are
	   linearly interpolated. Should yield the same forces as when the
	   var_morph option is disabled
	*/
    else if(fepvals->var_morph && fepvals->linear_test)
    {
		for (int i=0; i< md->nr; i++){
			for (int dim=0; dim<3; dim++){
				...
				// Linear test
				f[i][dim]  += (1. - lam) * fA[i][dim] + lam * fB[i][dim];
				fA[i][dim] = 0.;
				fB[i][dim] = 0.;
			}
		}
...
/* The total energies that differ between A and B have been collected separately.
 * Combine with non-lambda dependent energy terms
 */
void reduce_AB_energies(gmx_enerdata_t *enerd, real *lambda, t_lambda *fepvals){
	...
	real Va = Va_get[0] + enerd->term[F_EPOTA];
	real Vb = Vb_get[0] + enerd->term[F_EPOTB];
	...
	if(lam < 0.000001)
	{
		enerd->term[F_EPOT] += Va;
	}
	else if (lam > 0.999999)
	{
		enerd->term[F_EPOT] += Vb;
	}
	...
	else if (fepvals->var_morph && ! fepvals->linear_test)
	{
		...
		enerd->term[F_EPOT] += -kbT / s * log(gA + gB);
	}
	else if(fepvals->var_morph && fepvals->linear_test==1)
	{
        enerd->term[F_EPOT] = (1.-lam) * Va + lam * Vb;
	}
}
              ...
              const DDBalanceRegionHandler             &ddBalanceRegionHandler)
{
   ...
   //write_dhdl_to_log(fplog, enerd, *inputrec->fepvals);
    ...
    ForceOutputs forceOut = setupForceOutputs(fr, pull_work, *inputrec, std::move(force), std::move(forceA), std::move(forceB), stepWork, wcycle);
   ...
   //write_dhdl_to_log(fplog, enerd, *inputrec->fepvals);
    ...
    if (fr->efep != efepNO)
    {
         ...
         */
    	//write_forces_to_log(fplog, mdatoms, &forceOut, step);
    ...
    // forces need to be reduced first as these, for var_morph, depend on the energy terms
    if(inputrec->fepvals->var_morph){
        reduce_AB_f_to_f(mdatoms, &forceOut, enerd, lambda.data(), inputrec->fepvals);
        reduce_AB_energies(enerd, lambda.data(), inputrec->fepvals);
    }
