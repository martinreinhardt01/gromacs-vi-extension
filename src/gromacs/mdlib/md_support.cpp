                     ...
                     gmx_bool *bSumEkinhOld, const int flags)
{
    ...
    if ((bEner || bPres) && fr->dispersionCorrection)
    {
        ...
        if (bEner)
        {
        	...
        	{
        		enerd->term[F_EPOTA] += correction.energyA;
        		enerd->term[F_EPOTB] += correction.energyB;
        		if( ir->fepvals->sc_alpha != 0 )
        		{
        			enerd->term[F_VM_LIN_CORR] += correction.energyB - correction.energyA;
        		}
