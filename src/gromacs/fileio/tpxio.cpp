                       ...
                       int                       file_version)
{
    ...
    serializer->doInt(&fepvals->var_morph);
    serializer->doReal(&fepvals->deltag_est);
    serializer->doReal(&fepvals->smoothing);
    serializer->doInt(&fepvals->linear_test);
                    ...
                    int                       ePullOld)
{
    ...
    if (file_version < 95)
    {
        ...
        {
            fprintf(stderr, "NOTE: pull-geometry=position will be no longer supported in GROMACS 5 and later!\n");
        }
        if (eGeomOld > epullgDIRPBC)
        {
            eGeomOld -= 1;
                     ...
                     t_inputrec       *ir)
{
    ...
    /* Communicate the file version information to g_submit via the ir->userint1 parameter */
    if (serializer->reading())
    {
        ir->userint1 = tpx->fileVersion;
    }
