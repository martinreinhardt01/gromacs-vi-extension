...
/*****************************************************************************************/
    ...
    while (!bLastStep)
    {
        ...
        //write_dhdl_to_log(fplog, enerd, *ir->fepvals);
        if (shellfc)
        {
       ...
       //write_dhdl_to_log(fplog, enerd, *ir->fepvals);
        ...
        if (ir->efep != efepNO && !EI_VV(ir->eI))
        {
            ...
            /*if (ir->fepvals->var_morph)
            {
            	// linear sum_dhdl should not contain any non-zero values anymore in the var_morph case
            	// will therefore be checked in sum_dhdl_var_morph
            	sum_dhdl_var_morph(enerd, state->lambda, ir->fepvals);
            	write_energies_to_file(fplog, enerd, state->lambda, ir->fepvals, bDoDHDL);
            }
            */
            //write_dhdl_to_log(fplog, enerd, *ir->fepvals);
