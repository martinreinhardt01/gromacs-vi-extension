        ...
        if (stepWork.computeDhdl)
        {
            for (int i = 0; i < efptNR; i++)
            {
                ...
                for (int t = 1; t < bt->nthreads; t++)
                {
                    dvdl[i] += f_t[t]->dvdl[i];
                    if (f_t[t]->dvdl[i] != 0)
                    	std::cout << "Non zero at FLAG X at i = " << i << std::endl;
                }
            }
        }
    }
                 ...
                 const gmx::StepWorkload &stepWork)
{
    ...
    //write_forces(md, forceOutputs, 4);
    ...
    //write_forces(md, forceOutputs, 6);
    ...
    //write_forces(md, forceOutputs, 12);
                ...
                const gmx::StepWorkload       &stepWork)
{
    ...
    //write_forces(md, forceOutputs, 2);
    ...
    //write_forces(md, forceOutputs, 3);
