...
struct gmx_pme_pp {
    ...
    std::vector<gmx::RVec>      fA;
    std::vector<gmx::RVec>      fB;
                ...
                t_inputrec *ir, PmeRunMode runMode)
{
    ...
    do /****** this is a quasi-loop over time steps! */
    {
        ...
        do
        {
            ...
            if (atomSetChanged)
            {
                ...
                if (pme_pp->useGpuDirectComm)
                {
                    ...
                    // This rank will have data pulled from PP rank, so needs to send the remote receive address.
                    rvec* d_f = reinterpret_cast<rvec*> (pme_gpu_get_device_f(pme));
        ...
        else
        {
            ...
            gmx_pme_do(pme, pme_pp->x, pme_pp->f, pme_pp->fA, pme_pp->fB,
					   ...
					   &output.Va_, &output.Vb_,
                       lambda_q, lambda_lj, &dvdlambda_q, &dvdlambda_lj,
                       pmeFlags, ir->fepvals->var_morph);
