              ...
              gmx_ewald_tab_t  *et)
{
    ...
    else
    {
        ...
        *dvdlambda += scaleRecip*(energy_AB[1] - energy_AB[0]);
        //if ( scaleRecip*(energy_AB[1] - energy_AB[0]) != 0 )
        //	std::cout << "Warning. dvdl not zero in ewald.cpp" << std::endl;
    }
...

{
    ...
    if (MASTER(cr))
    {
        ...
        *dvdlambda += -vol*(qs2B - qs2A)*fac;
        ...
        //if( -vol*(qs2B - qs2A)*fac != 0)
        	//std::cout << "WARNING: dvdl contribution in ewald.cpp not zero. " << vol*(qs2B - qs2A)*fac << "  \n \n";
        	//showed!
